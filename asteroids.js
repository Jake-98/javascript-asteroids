...
function SetupCanvas() {
  ...
  // Store all possible keycodes in an array so that multiple keys can work at the same time
...
class Ship {
  ...
  Update() {
    // Get current direction the ship is facing
    ...
    // Place ship on opposite side if it goes off screen
    ...
    // Slow ship speed when not holding a key
    this.velX *= 0.99;
    this.velY *= 0.99;

    // Change value of x & y to account for "air friction"
    this.x -= this.velX;
    this.y -= this.velY;
  }
  Draw() {
...
class Asteroid {
  constructor(x, y, radius, level, collisionRadius) {
    ...
    // Used to decide if this asteroid can be broken into smaller parts
    this.level = level || 1;
  }
  Update() {
    let radians = (this.angle / Math.PI) * 180;
...
// Handles drawing life ships in top right corner of screen
...
function Render() {
  ...
  // If there are no lives left, signal game over
  if (lives <= 0) {
    // Remove event listeners to stop getting keyboard input after Game Over
  ...
  // Check for bullet collision with asteroidcollision with bullet and asteroid
  ...
  if (asteroids.length !== 0) {
    for (let j = 0; j < asteroids.length; j++) {
      asteroids[j].Update();
      // Pass j to track which asteroid points to store
      asteroids[j].Draw(j);
    }
  }
